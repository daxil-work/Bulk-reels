/* ════════════════════════════════════════════════════════════════════════
   FREE PHOTOSHOOT REEL — single-source canvas renderer.
   The SAME draw() powers the live preview and the MP4 export, so what you
   see is exactly what downloads. Exposes window.FP.
   ════════════════════════════════════════════════════════════════════════ */
(function () {
  // ── math / easing ───────────────────────────────────────────────────────
  ...
  // ── asset paths ──────────────────────────────────────────────────────────
  const BEFORE = "assets/before.png";
  const LOGO = "assets/logo.jpeg";
  const LOGO_GLOW = "assets/logo-glow.png";
  const POOL = ["uploads/1.png", "uploads/2.png", "uploads/3.png", "uploads/4.png",
    "uploads/5.png", "uploads/6.png", "uploads/7.png", "uploads/8.png", "uploads/9.png"];
  const REVEAL = ["uploads/5.png", "uploads/1.png", "uploads/8.png"];
  const GALLERY = ["uploads/1.png", "uploads/2.png", "uploads/5.png", "uploads/7.png", "uploads/8.png", "uploads/9.png"];

  const ALL_FONTS = ["Cormorant Garamond", "Playfair Display", "DM Serif Display", "Lora", "Marcellus", "Jost", "Poppins", "Montserrat"];
  const SERIF_FONTS = ["Cormorant Garamond", "Playfair Display", "DM Serif Display", "Lora", "Marcellus"];
  ...
  const DEFAULT_DUR = [3.5, 6.5, 15, 15, 9.5, 10];
  function scenesOf(t) {
    const num = (v, d) => (typeof v === "number" && !isNaN(v)) ? v : d;
    const d = [
  ...
  function durationOf(t) { const sc = scenesOf(t); return sc[sc.length - 1].end; }
  ...
  function glowLogo(ctx, cfg, cx, cy, size, alpha) {
    const img = getImg(cfg, LOGO_GLOW) || getImg(cfg, LOGO);
    ctx.save();
  ...
  function scHook(ctx, lt, dur, cfg) {
    ...
    const img = getImg(cfg, t.hookImg || BEFORE);
  ...
  function scProblem(ctx, lt, dur, cfg) {
    ...
    const before = getImg(cfg, BEFORE);
    const after = getImg(cfg, t.s2studio || "uploads/4.png");
  ...
  window.FP = { draw, scenesOf, durationOf, buildFR, ALL_FONTS, POOL, BEFORE, LOGO, LOGO_GLOW, REVEAL, GALLERY,
    util: { clamp, lerp, luma, hexRgb }, DEFAULT_DUR };
})();
